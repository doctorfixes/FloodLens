// zonecheck-geocoder.test.ts - Census and Google geocoder tests for the
// production zonecheck function (Deno). Mirrors tests/geocoder.test.ts, which
// only exercises the legacy determine-zone copy.
...
Deno.test("Census geocoder returns result on success", async () => {
  ...
  try {
    const { geocodeCensus } = await import(
      "../supabase/functions/zonecheck/geocoder.ts"
    );
...
Deno.test("Census geocoder returns null on non-OK status", async () => {
  const originalFetch = globalThis.fetch;
...
Deno.test("Google geocoder returns null when API status is not OK", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = mockFetch({ status: "ZERO_RESULTS", results: [] });
